...
function patchIfString(nodes) {
  ...
  for (const n of nodes) {
    if (n.name === '¿Superó Umbral?') {
      ...
      modified = true;
      console.log('Patched ¿Superó Umbral? to string comparison');
    }
  }
  return modified;
}
...
db.all('SELECT id, name, nodes FROM workflow_entity', (err, workflows) => {
  ...
  db.all('SELECT versionId, workflowId, nodes FROM workflow_history', (errH, histories) => {
    ...
    setTimeout(() => {
      console.log('Finished updating string condition in SQLite!');
      process.exit(0);
    }, 1000);
  });
});
